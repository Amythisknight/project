...
void i2cspm_app_process_action(void)
{
  ...
  if (true) {
    ...
    // Use PRId32 for signed, PRIu32 for unsigned
    // 2. Format into separate buffers
    int hum_len = sprintf(hum_buffer, "Humidity Relative = %" PRIu32 "%%", relative_humidity);
    int temp_len = sprintf(temp_buffer, "Temperature = %" PRId32 "C", temperature);

    // 3. Print using your printc function (or printf)
    printc(hum_buffer, hum_len);
    printc(enter, sizeof(enter));
    printc(temp_buffer, temp_len);
    printc(enter, sizeof(enter));
#if SL_SIMPLE_LED_COUNT > 1
    ...
    if (temperature > high_limit) {
      char hightemp[] = "Temperature is high.\r\n";
      printc(hightemp,sizeof(hightemp));

    } else if (temperature < low_limit) {
        char lowtemp[] = "Temperature is low.\r\n";
      printc(lowtemp,sizeof(lowtemp));
    }
